...
export class SVGPolygonSubboard extends SVGPolygon {
    ...
    private isNeigh(row: number, col: number): Boolean {
        switch(this.sides) {
            case 3:
                if (Math.abs(col - this.col) === 1 && row == this.row) return true;
                if (col == this.col) {
                    if ((this.row + this.col) % 2 === 0 && row === this.row - 1) return true;
                    if ((this.row + this.col) % 2 === 1 && row === this.row + 1) return true;
                }
                return false;
            case 4:
                return (Math.abs(row - this.row) + Math.abs(col - this.col) === 1);
            case 6:
                if (row === this.row && Math.abs(col - this.col) === 1) return true;
                if (Math.abs(row - this.row) === 1) {
                    if (this.row % 2 === 0) {
                        if (col === this.col - 1 || col === this.col) return true;
                    } else {
                        if (col === this.col || col === this.col + 1) return true;
                    }
                }
                return false;
            default:
